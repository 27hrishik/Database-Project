<template>
    <div class="box">
        <h1><strong>Create Operator Account</strong></h1>
        <div class="box">
            <h2><strong>Login Details</strong></h2>
            <div class="field">
            <label class="label">Operator Username</label>
            <div class="control">
                <input class="input" type="text" v-model="operatorUsername" placeholder="e.g abc21travels">
            </div>
            </div>
            <div class="field">
            <label class="label">password</label>
            <div class="control">
                <input class="input" type="password" v-model="password" placeholder="e.g ax123#ads">
            </div>
            </div>
        </div>
        <div class="box">
            <h2><strong>General</strong></h2>
            <div class="field">
            <label class="label">Operator Name</label>
            <div class="control">
                <input class="input" type="text" v-model="operatorName" placeholder="e.g SRS Travels">
            </div>
            </div>
            <div class="field">
            <label class="label">Mobile No.</label>
            <div class="control">
                <input class="input" type="text" v-model="mobile" placeholder="e.g 7259212564">
            </div>
            </div>
            <div class="field">
            <label class="label">Email</label>
            <div class="control">
                <input class="input" type="text" v-model="email" placeholder="e.g alex.smith@gmail.com">
            </div>
            </div>
            <div class="field">
            <label class="label">Address</label>
            <div class="control">
                <input class="input" type="text" v-model="mobile" placeholder="e.g 1st Block,NY">
            </div>
            </div>
            <div class="field">
            <p class="control">
            <button class="button is-success" @click="createAccount">
                Create Operator Account
            </button>
            <button class="button is-danger" @click="goToHome">
                Cancel
            </button>
            </p>
            </div>
        </div>
        <hr>
    </div>
</template>

<script>
export default {
    name:'',
    data:function(){
        return{
            operatorName:'',
            mobile:"",
            email:"",
            address:"",
            operatorUsername:"",
            password:""
        }
    },
    methods:{
        createAccount:function(){

        },
        goToHome:function(){
            
        }
    }
}
</script>

<style scoped>
.input{
    width: 20%;
}
</style>